...
public:
    ...
    double first[960][2]; // weight of edges between 1st and 2nd layer
    double error,err;
    double mom;
    double eta;
    double gamma;
    double out1[960],O[2]; // output of each layer
    ...
    void sigmoid_2() // calculates output of sigmoid units in output layer
    {
        double sum=0.00;
        int i,k=0;
        while(k<2)
    ...
    shades() //constructor sets weights between -0.5 and 0.5 randomly , also sets network parameters like learning rate , momentum
    {
        int i,j;
        D.Init(1);
        mom=0.3;
...
int main()
{
    ...
    for(int z=1;true;z++)
    {
        ...
        p.validate();
        error = p.error;
        printf("Accuracy after %d epochs on validation set %lf%%\n",z*50,(1-error)*100);
        p.selfValidate();
        error = p.error;
        printf("Accuracy after %d epochs on training set %lf%%\n",z*50,(1-error)*100);
        if(perror <= error)
        {
            for(i=0;i<960;i++)
                for(j=0;j<2;j++)
            ...
            p.validateFinal();
            error = p.error;
            printf("Accuracy on final testing set %lf%%\n",(1-error)*100);
            // char fl[] = "pose.csv";
